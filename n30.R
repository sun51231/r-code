...
get_pvalues <- function(n) {
  ...
  mu_y4<--50.735178*(ncol(dat)/(nrow(dat))^2)+4.788150*(ncol(dat)/nrow(dat))+0.003553*nrow(dat)-0.183627*m-2.639546  ## g1'
...
#####
# 安全封装函数（记录失败次数）
failure_count <- 0  # 初始化计数器

get_pvalues_safe <- function(n) {
  tryCatch({
    get_pvalues(n)  # 原始获取p值的函数
  }, error = function(e) {
    # 遇到不收敛时增加计数器
...
# 运行模拟（使用future_replicate或replicate）
...
cat("不收敛的样本数量：", failure_count, "/1000\n")
# 转换为矩阵（27列 x N行）
result_matrix <- do.call(rbind, valid_results)
# 验证维度
cat("有效结果维度：", dim(result_matrix), "\n")
...
##
L1<-data.frame(matrix(nrow = 1,ncol = 12))
colnames(L1)<-c("T_mlb","T_rmlb","T_rls","T_CTrls","T_F","T_F_c1","T_F_c2","T_F_cr","T_CsF","T_CsF_c1","T_CsF_c2","T_CsF_cr")
L1[1,]<-rowMeans(result_matrix_t[1:12,])
tab_df(L1,digits = 4)
