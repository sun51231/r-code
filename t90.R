...
get_pvalues <- function(n) {
  ...
  ##
  mu_y0<-2.015910*(m/ncol(dat))+1.291412*(ncol(dat)/nrow(dat))-0.278377*m+0.036066*ncol(dat)-2.393643
  T_F_c<-(fitmeasures(fit, "chisq")/nrow(dat)-tau-mu_y0*v)/v
  pT_F_c<-1-pnorm(T_F_c,0,1)   
  mu_y2<-0.867809/m+2.912802*(ncol(dat)/nrow(dat))-0.158941*m+0.014435*ncol(dat)+0.003473*nrow(dat)-3.146526
  T_F_c1<-(fitmeasures(fit, "chisq")/nrow(dat)-tau-mu_y2*v)/v
  ...
  ##
  mu_y1<-1.531018*(ncol(dat)/nrow(dat))-0.237301*m+0.029336*ncol(dat)-2.035224
  T_CsF_c<-(fitmeasures(fit, "chisq.scaled")/nrow(dat)-tau-mu_y1*v)/v
  pT_CsF_c<-1-pnorm(T_CsF_c,0,1) 
  mu_y3<-0.844786/m+3.391213*(ncol(dat)/nrow(dat))-0.161134*m+0.014666*ncol(dat)+0.003517*nrow(dat)-3.146773
  ...
  T_CsF_cr<-(fitmeasures(fit1, "chisq.scaled")/nrow(dat)-tau-mu_y3*v)/v
  pT_CsF_cr<-1-pnorm(T_CsF_cr,0,1) 
  
  ##
  Tlw<-(2*CT_rls/ncol(dat)-2*df/ncol(dat))/(2*sqrt(2*df)/ncol(dat))
  pTlw<-1-pnorm(Tlw,0,1)     # 16
  
  c(chisq_mlb,chisq_rmlb,T_rls,CT_rls,T_F,T_F_c,T_F_c1,T_F_cr,T_CsF,T_CsF_c,T_CsF_c1,T_CsF_cr,p_mlb,p_rmlb,p_rls,p_CTrls,pT_F,pT_F_c,pT_F_c1,pT_F_cr,pT_CsF,pT_CsF_c,pT_CsF_c1,pT_CsF_cr,pTlw,pNTML,pSB,pEBAF,pEBA2,pEBA2J,pEBAA)
}
...
colnames(L)<-c("p_mlb","p_rmlb","p_rls","p_CTrls","pT_F","pT_F_c","pT_F_c1","pT_F_cr","pT_CsF","pT_CsF_c","pT_CsF_c1","pT_CsF_cr","p_ml","pNTML","pSB","pEBAF","pEBA2","pEBA2J","pEBAA")
