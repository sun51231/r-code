...
library(gtools)  
library(regsem)
library(semTests)    ## Version 0.7.1 
...
lambda <- cbind(
  c(loadings, rep(0, 40)),
  c(rep(0, 5), loadings, rep(0, 35)),
  c(rep(0, 10), loadings, rep(0, 30)),
  c(rep(0, 15), loadings, rep(0, 25)),
  c(rep(0, 20), loadings, rep(0, 20)),
  c(rep(0, 25), loadings, rep(0, 15)),
  c(rep(0, 30), loadings, rep(0, 10)),
  c(rep(0, 35), loadings, rep(0, 5)),
  c(rep(0, 40), loadings)
)
phi <- matrix(c(1,.3,.3,.3,.3,.3,.3,.3,.3,
                .3,1,.3,.3,.3,.3,.3,.3,.3,
                .3,.3,1,.3,.3,.3,.3,.3,.3,
                .3,.3,.3,1,.3,.3,.3,.3,.3,
                .3,.3,.3,.3,1,.3,.3,.3,.3,
...
get_pvalues_optimized <- function(n) {
  tryCatch({
    # 
    p <- ncol(Sigma.sqrt)
    w_raw <- matrix(rchisq(n * (p+1), df = 4), nrow = n, ncol = p+1)
    w <- (w_raw - 4) / (2 * sqrt(2))
    ...
    # Initialize result vector (15 NA values)
    result <- rep(NA, 15)
    names(result) <- c("p_ml","p_mlb", "p_rmlb", "p_rls", "p_Csrls", 
    ...
    if (fit_converged && !is.null(fit)) {
      ...
      T_ml <- fitmeasures(fit, "chisq")
      
      
      result["p_ml"] <- 1 - pchisq(T_ml, df)
      ...
      if (!is.null(fit@test$browne.residual.nt.model)) {
        ...
        result["p_Csrls"] <- 1 - pchisq(CT_rls, df)
      }
    ...
    if (!is.null(fit)) {
      tryCatch({
        pv_EBA <- semTests::pvalues(fit,tests = c( "pEBA4_RLS","pOLS_RLS"))   ### tests = c("SB_UG_RLS", "pEBA2_UG_RLS", "pEBA4_RLS", "pEBA6_RLS", "pOLS_RLS")
        # Ensure pv_EBA has sufficient length.
        if (length(pv_EBA) >= 2) {
          result["pEBA4_rls"] <- pv_EBA[1]
          result["pols_2_rls"] <- pv_EBA[2]
        }
      }, error = function(e) {
...
fit_non_converged_count <- 0
# 
res <- future_replicate(n_simulations, get_pvalues_optimized(50), future.seed = TRUE)

# Transform results into a matrix (15 rows x n_simulations columns).
result_matrix <- matrix(unlist(res), nrow = 15, byrow = FALSE)
rownames(result_matrix) <- c("p_ml", "p_mlb", "p_rmlb", "p_rls", "p_Csrls", 
                             "pT_F", "pT_F_c1", "pT_F_c2", "pT_F_cr",
                             "pT_CsF", "pT_CsF_c1", "pT_CsF_c2", "pT_CsF_cr",
                             "pEBA4_rls", "pols_2_rls")
...
# 
cat("\n============== Table output ===============\n")
L <- data.frame(matrix(nrow = 1, ncol = 15))
